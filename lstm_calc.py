...
class reg_linear():
	...
	def calc_reg(self):
		...
		try:
			...
			train, test = (train_test_split(kcloses, train_size=0.95))
			...
			model = Sequential()	
			model.add(LSTM(210, input_shape=(train.shape[0], 1), use_bias=True))
			model.add(Dropout(0.55))
			model.add(Dense(15, activation='relu', use_bias=True, bias_initializer=""))
